...
public:
  ...
  int32_t AddIntersection(float x, float y) {
    ...
    m_nodes[id] = {static_cast<int32_t>(x * Config::FP_MULT),
                   static_cast<int32_t>(y * Config::FP_MULT),
                   0,
                   0,
                   0,
                   -1,
  ...
  void AddRoad(int32_t f, int32_t t, float traffic) {
    int32_t dx = std::abs(m_nodes[t].x_fp - m_nodes[f].x_fp),
            dy = std::abs(m_nodes[t].y_fp - m_nodes[f].y_fp);
    int32_t mn = dx < dy ? dx : dy, mx = dx > dy ? dx : dy;

    int32_t distFP = mx + ((Config::SQRT2_MINUS_1_FP * mn) >> 10);
    int32_t edgeId = m_eCount++;
    m_edges[edgeId] = {t, static_cast<int32_t>(distFP * traffic),
                       m_nodes[f].headEdge};
    m_nodes[f].headEdge = edgeId;
  }
  void AddTwoWay(int32_t a, int32_t b, float t) {
    AddRoad(a, b, t);
    AddRoad(b, a, t);
  }
...
class UrbanMinHeap {
  CityIntersection *m_pool;
  int32_t m_cap, m_size, *m_heap, *m_pos;

public:
  void Init(int32_t cap, CityIntersection *pool,
...
public:
  void Init(CityMap *map, memory::ArenaAllocator &arena) {
    m_map = map;
  ...
  UrbanPathResult RouteTraffic(int32_t startId, int32_t targetId) {
    ...
    int32_t dx = std::abs(nodes[startId].x_fp - nodes[targetId].x_fp),
            dy = std::abs(nodes[startId].y_fp - nodes[targetId].y_fp);
    nodes[startId].h = (dx > dy ? dx : dy) +
                       ((Config::SQRT2_MINUS_1_FP * (dx < dy ? dx : dy)) >> 10);
      ...
      for (int32_t e = curr.headEdge; e != -1; e = edges[e].nextEdge) {
        ...
        if (next.state == 0 || g < next.g) {
          ...
          int32_t ndx = std::abs(next.x_fp - nodes[targetId].x_fp),
                  ndy = std::abs(next.y_fp - nodes[targetId].y_fp);
          next.h = (ndx > ndy ? ndx : ndy) +
                   ((Config::SQRT2_MINUS_1_FP * (ndx < ndy ? ndx : ndy)) >> 10);
          next.parent = cId;
...
private:
  inline void InitNode(CityIntersection &n) {
