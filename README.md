...
--------------------------------------------------------------------------------
BM_Halo_EscapeRaycast      0.318 ns        0.317 ns   2213375746 items_per_second=3.1528G/s
...
HOW TO READ REPORT
1. PERFORMANCE METRICS (REPORT HEADER)

After executing the simulation, the system outputs a hardware-level performance report. Here is how to interpret the data:

Breaking Speed Limit (ms): This is the average time taken for a single Raycast operation. In our optimized M1 environment, this typically reaches sub-nanosecond levels (e.g., 0.00000001012 ms). It represents the "Reaction Latency" of the rescue drone.

Raycast Impact Vector: Displayed as (X, Y). This is the exact coordinate where the drone's primary sensor first detects an obstacle in its current heading.
...
2. TACTICAL MAP & ENTITY RECOGNITION
...
🤖 [Robot]: The current position of the H.A.L.O. equipped rescue unit.

❤️ [Victim]: The high-priority target (Individual needing rescue) located at the far end of the map.

✨ [Quantum Path]: The calculated "Safe Line." This represents the optimal trajectory the drone must follow to bypass all hazards.

🎯 [Impact Point]: The specific location where the current Raycast has hit an obstacle.

Environmental Hazards (Layers 0-14):

██ [Wall/Solid]: Impenetrable structural debris or permanent boundaries.

💥 [Explosion]: High-heat zones or active blast radius.

🔥 [Fire]: Thermal hazards that require immediate evasion.

⚡ [Electric]: High-voltage areas or electrical discharge zones.

🧲 [Magnetic]: Electromagnetic interference zones that can scramble drone sensors.

🛸 [UFO/Unknown]: Unidentified moving objects or dynamic aerial threats.

🦅 [Avian/Drone]: Biological or mechanical aerial obstacles.

☁️ [Gas/Smoke]: Low visibility or toxic zones.

XX [Hazard]: General unidentified structural threats.

.  [Void]: Clear, traversable air space.

3. NAVIGATION LOGIC

The H.A.L.O. engine processes the environment in a single clock cycle. By analyzing the "Quantum Path" (✨), the unit can navigate through the "Hell Matrix" without physical contact with any hazard symbols.

If the Impact Vector (🎯) appears on the path, the system immediately recalculates the vector to ensure the drone maintains a 100% survival rate during high-speed extraction missions.